...
function ShippingInvoice({ productID, onUploaded }) {
    ...
    const onSubmit = async (data) => {
        ...
        form.append('shippingInvoice', data.shippingInvoice[0])

        try {
            setLoading(true)
            await axios.put(`/orders/${productID}/shipping`, form, {
        ...
        <div className="mx-auto bg-white p-6 rounded-lg shadow-sm">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                <div>
                    ...
                    <label htmlFor="shippingInvoice" className="flex items-center gap-4 p-4 border-2 border-dashed rounded-lg cursor-pointer hover:border-accent">
                    ...
                    <input 
                        ...
                        className="hidden" 
                        {...register('shippingInvoice', { onChange: onFiles })} 
                    />
                ...
                <div className="flex justify-end">
                    <button 
                        type="submit" 
                        disabled={loading} 
                        className="bg-[#FBBC04] hover:brightness-95 text-white px-4 py-2 rounded-md"
                    >
                        {loading ? "Uploading..." : "Submit Shipping Invoice"}
                    </button>
                </div>
            </form>
        </div>
