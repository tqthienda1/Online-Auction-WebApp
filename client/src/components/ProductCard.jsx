    ...
    <div className="relative flex flex-col justify-between items-center border border-gray-200 rounded-lg shadow-sm bg-white p-4 transition-shadow hover:shadow-xl">
          ...
          <div className="w-full">
            <div className="aspect-video mb-3 flex items-center justify-center w-full">
              <Link key={product.id} to={`/products/${product.id}`}>
                <img src={product.productAvt} alt={product.productName} className="max-h-40 w-full object-contain" />
              </Link>
            </div>
            <div className="flex items-center justify-center text-gray-500 mb-2">
              <RiAuctionLine className="w-7 h-7 mr-2" />
              <span className="text-brand font-semibold">{product.totalBid || 0}</span>
            </div>
            <h3 className="text-center font-semibold text-gray-800 mb-1 truncate">{product.productName}</h3>
            <p className="text-center text-yellow-500 font-bold text-lg mb-4">${product.currentPrice}</p>
            <div className="text-sm text-gray-600 space-y-1 mb-4">
              <p className="flex justify-between">
                <span>Highest bidder:</span>
                <span className="font-medium">{maskUsername(product.highestBidder?.username) || "None"}</span>
              </p>
              <p className="flex justify-between">
                <span>Buy now:</span>
        ...
        <div className="w-[250px]">
          <div className="flex flex-col aspect-video w-full items-center justify-center">
            <Link to={`/products/${product.id}`} className="w-full flex items-center justify-center">
              <img src={product.productAvt} alt={product.productName} className="max-h-40 max-w-full object-contain" />
            </Link>
            <h3 className="text-center font-semibold text-gray-800 mb-1 truncate mt-5">{product.productName}</h3>
            <div className=" space-y-1 mb-4 w-full">
              <p className="flex justify-between items-center">
                <span className="text-sm text-gray-600">Current price:</span>
                <span className="text-lg font-bold text-yellow-400">{product.currentPrice.toLocaleString()}₫</span>
              </p>
        ...
        <div className="w-[250px]">
          <div className="flex flex-col aspect-video w-full items-center justify-center">
            ...
            <div className=" space-y-1 mb-4 w-full">
              <p className="flex justify-between">
                <span className="text-sm text-gray-600">Current price:</span>
                <span className="text-lg font-bold text-yellow-400 flex items-center justify-center">{product.currentPrice.toLocaleString()}₫</span>
              </p>
              <p className="flex justify-between">
      ...
      {(showType === 4 || showType === 6) && (
        <div className="w-[250px] cursor-pointer">
          <div className="flex flex-col aspect-video w-full items-center justify-center">
            <Link to={showType === 4 ? `/buyer/payment/${product.id}` : `/seller/payment/${product.id}`}>
              <img src={product.productAvt} alt={product.productName} className="max-h-40 max-w-full object-contain" />
            </Link>
            <h3 className="text-center font-semibold text-gray-800 mb-1 truncate mt-5">{product.productName}</h3>
            <div className=" space-y-1 mb-4 w-full">
              <p className="flex justify-between">
                <span className="text-sm text-gray-600">{showType === 4 ? "Won price:" : "Sold price:"}</span>
                <span className="text-lg font-bold text-yellow-400 flex items-center justify-center">{product.currentPrice.toLocaleString()}₫</span>
              </p>
              <p className="flex gap-1 text-gray-600 justify-between items-center">
                <span className="text-sm ">{showType === 4 ? "Seller:" : "Buyer:"}</span>
                <span className="text-sm font-semibold">{product.sellerName || product.buyerName || "N/A"}</span>
              </p>
              <p className="flex text-sm justify-between text-gray-500">
                <span>End:</span>
                <span>{new Date(product.endTime).toLocaleString()}</span>
            ...
            <Link to={showType === 4 ? `/buyer/payment/${product.id}` : `/seller/payment/${product.id}`} className="w-full">
            <button className="w-full flex items-center justify-center text-sm p-2 rounded-md cursor-pointer text-white font-semibold bg-yellow-500 hover:bg-yellow-600">
              Payment and rating
            </button>
            </Link>
