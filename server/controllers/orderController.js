...
export const updateBuyerInfo = async (req, res) => {
  try {
    ...
    if (!order.buyer || String(order.buyer.id) !== String(userID)) {
      return res.status(403).json({ message: 'Only the buyer can update payment info' })
    }
...
export const uploadShipping = async (req, res) => {
  try {
    ...
    if (!order.seller || String(order.seller.id) !== String(userID)) {
      return res.status(403).json({ message: 'Only the seller can upload shipping invoice' })
    }
...
export const confirmPaymentReceived = async (req, res) => {
  try {
    ...
    if (!userID) return res.status(401).json({ message: 'Authentication required' })
    if (!order.seller || String(order.seller.id) !== String(userID)) {
      return res.status(403).json({ message: 'Only the seller can confirm payment received' })
    }
...
export const confirmProductReceived = async (req, res) => {
  try {
    ...
    if (!userID) return res.status(401).json({ message: 'Authentication required' })
    if (!order.buyer || String(order.buyer.id) !== String(userID)) {
      return res.status(403).json({ message: 'Only the buyer can confirm product received' })
    }
...
export const cancelOrder = async (req, res) => {
  try {
    ...
    try {
      ...
      if (rateeID) {
        await ratingService.submitRating({ raterID, rateeID, productID, value: -1, comment: 'Người thắng không thanh toán' })
      }
    } catch (ratingErr) {
